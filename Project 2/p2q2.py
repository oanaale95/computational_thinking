...
def q2_v2 (p, picked_flags_by_players_dict, graph_flags):
    dict_solution_v1 = q2_v1(p, picked_flags_by_players_dict, graph_flags)

    for solution_of_player in dict_solution_v1.keys():

        min_distance_to_sp = dict_solution_v1[solution_of_player][-1].distance_to_vertex(graph_flags.startingPoint)

        return_to_sp_flags_list = []
        prev_vertex =  dict_solution_v1[solution_of_player][-1]
        distance_from_last_flag = 0

        for flag_vertex in reversed( dict_solution_v1[solution_of_player][:-1]):
            ...
            if distance_from_last_flag + flag_vertex.distance_to_vertex(graph_flags.startingPoint) < min_distance_to_sp:
                ...
                index_of_flag_vertex = dict_solution_v1[solution_of_player].index(flag_vertex)
                return_to_sp_flags_list = list(reversed(
                    dict_solution_v1[solution_of_player][index_of_flag_vertex:-1]))
        ...
        dict_solution_v1[solution_of_player].extend(return_to_sp_flags_list)

    return dict_solution_v1
...
def get_routes(p, v, flags, n):
    ...
    if v == 1:
        ...
        solution = q2_v2(p, picked_flags_by_players_dict, graph_flags)

    solution_list =[]
    for player_id in picked_flags_by_players_dict.keys():
        ...
        solution_list.append(players_and_flags_str[1:])
