...
class Graph:
    def __init__(self):
        ...
        #Initialize with Starting Point vertex
...
def generate_random_sample(flags_graph):
    '''
    :param flags_graph:
    :return: random sample of vertices from flags_graph as a list , size of random sample is half
...
def get_sample_of_unvisited_flags(flags_graph, depth=0):
    ...
    '''
    Returns a random sample of unvisited vertices from a given graph (flags_graph)
    '''
...
def v2(p, graph_flags):
    '''
       v2 represents a Greedy algorithm, where the distance between the current vertex and the starting point is
       calculated and compared against moving back on the same trail of nodes
